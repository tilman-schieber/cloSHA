;
; AES
;
; http://csrc.nist.gov/publications/fips/fips197/fips-197.pdf
;

(ns closha.aes)

(def gexp
  "Exponentiation chart for 0xe5, a generator of the Rjindael Galois Field GF(2^8).
   The i-th element of the vector corresponds to 0xe5^i.
   Source: http://www.samiam.org/galois.html"
  [])

(def glog
  "Logarithmic chart for 0xe5. The i-th position is n in the equation 0xe5^n=i
   For i=0 the value is undefined.
   Source: http://www.samiam.org/galois.html"
  [ nil ])

(defn gmul
  "multiplication in the Galois Field GF(2^8) using table lookup"
  [a b]
  (if (zero? (* a b)) 0 (gexp (mod (+ (glog a) (glog b)) 255))))

(defn ginv
  "the multiplicative inverse in the Galois Field GF(2^8) using table lookup"
  [a]
  (if (zero? a) 0 (gexp (- 255 (glog a)))))

(defn bit-rot-left
  [x n]
  (bit-and (bit-or (bit-shift-left x n) (bit-shift-right x (- 8 n))) 0xff))


;(def bit-rot-right #(bit-rot-left %1 (- 8 %2)))

(defn s-box-r
  "the affine transformation used to build the s-box"
  [x]
  (bit-xor x
           (bit-rot-left x 1)
           (bit-rot-left x 2)
           (bit-rot-left x 3)
           (bit-rot-left x 4)
           0x63))

(def s-box-r-1
 "the inverse affine transformation s-box ^ -1 = s-box-r ^ 3
  source: https://crypto.stackexchange.com/a/17931"
 (let [r s-box-r] (comp r r r)))

(def s-box
  "The Rjindael s-box"
  (vec (map (comp s-box-r ginv) (range 0 256))))

(def s-box-1
  "The inverted s-box"
  (vec (map (comp ginv s-box-r-1) (range 0 256))))

(def s-box-1-deprecated
  "calculating the inverted s-box by walking through the s-box"
  (loop [s (vec (take 256 (repeat 0)))
       i 0]
       (if (= i 256) s (recur (assoc s (s-box i) i) (inc i)))))

(defn sub-bytes
  "SubBytes() according to FIPS-197 p.15f"
  [state]
  (map s-box state))

(defn rotate
  "Take a collection and rotates it n steps left. If n is negative,
  the collection is rotated right."
  [n col]
  (let [c (count col)]
    (take c (drop (mod n c) (cycle col)))))


(defn shift-rows
  "ShiftRows() according to FIPS-197 p.17"
  [state]
  )
